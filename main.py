...
async def run_telegram_bot_async():
    ...
    try:
        ...
        # Create application with retry configuration
        ...
        # Start polling with drop_pending_updates to clear any existing updates
        await application.bot.delete_webhook(drop_pending_updates=True)
        
        if application.updater:
            await application.updater.start_polling(drop_pending_updates=True)
        else:
            logger.error("Updater is not available")
        ...
        while True:
            bot_status["last_seen"] = time.time()
            await asyncio.sleep(10)  # Reduced frequency to avoid spam
